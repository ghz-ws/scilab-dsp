...
D=4         //bits/symbol. 1,2,4,6,8
B=16;       //filter bit resolution

//calc discrete time impulse response
T=1/Baud;
Ts=1/Fs;
for i=1:Fs/Baud*10
    t=i*Ts;
...
for i=1:size(coe)(1)-1
    tp=string(int(coe(i)*2^B))+", ";
    txt=txt+tp;
end
txt=txt+string(int(coe(size(coe)(1))*2^B))+"}";
disp(txt);
...
for i=1:L/rate
    ...
    for j=1:rate
        ...
        elseif D==8 //8bit/symbol 256QAM
            i_base=temp(1)+2*temp(2)+4*temp(3)+8*temp(4);   //0-15
            q_base=temp(5)+2*temp(6)+4*temp(7)+8*temp(8);
            x1(rate*(i-1)+j)=(i_base-1.5)/1.5;
            if j==1
                I(rate*(i-1)+j)=(i_base-7.5)/7.5;
                Q(rate*(i-1)+j)=(q_base-7.5)/7.5;
            else
                I(rate*(i-1)+j)=0;
                Q(rate*(i-1)+j)=0;
            end
